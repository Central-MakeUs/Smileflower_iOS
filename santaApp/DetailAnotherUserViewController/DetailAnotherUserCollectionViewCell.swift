...
class DetailAnotherUserCollectionViewCell: UICollectionViewCell {
    static let reuseIdentifier = String(describing: DetailAnotherUserCollectionViewCell.self)
    let viewbackgroundView = UIView()
    
    var detailAnotherUserViewController : DetailAnotherUserViewController?
    var userIdx : Int?
    var flagIdx : Int?
    var pictureIdx : Int?
...
extension DetailAnotherUserCollectionViewCell {
    func successDataApiReport() {
        print("성공")
        detailAnotherUserViewController?.presentAlert(title: "신고가 완료되었습니다.")
    }
    func failuerDataApi(_ message : String) {
        detailAnotherUserViewController?.presentAlert(title: message)
    }
}
